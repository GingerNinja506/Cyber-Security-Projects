## Message for Recruiter

Thank you for reviewing my GitHub portfolio.

I have been learning cybersecurity for about a year in an on-and-off way, mainly because I was still focused on my Scrum Master role. At this point, I am fully committed to transitioning into security, with a strong focus on SOC and defensive analysis.

I have already completed most of the original TryHackMe SOC Level 1 path. Recently, the path was updated with new content, so I decided to go through it again from the beginning — this time documenting every key step, technique, and conclusion here in this repository.

Please note: the portfolio is a work in progress, but this does not mean I lack knowledge in areas not yet documented. Having already obtained my Security+ certification, I have foundational knowledge in many of these areas, which accelerates my documentation process. I am systematically adding new analyses, phishing investigations, alert triage cases, and hands-on labs.

My goal is to present a clear and practical demonstration of my skills, mindset, and ability to learn fast — not just theory, but real investigation workflow, reasoning, and technical steps.

Thank you for your time.
